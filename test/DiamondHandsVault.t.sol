...
contract DiamondHandsVaultTest is Test {
    ...
    uint256 ethAllTimeHigh = 495_373_000_000;
    ...
    function setUp() public {
        vm.createSelectFork(vm.envString("MAINNET_RPC_URL"), 24_703_610);
    ...
    function test_deposit_shouldSuccessfullyDeposit() public {
        ...
        vm.expectEmit();
        emit DiamondHandsVault.ETHDeposited(depositor1, 1 ether, 813_127_203_918_803_466);

        vm.prank(depositor1);
        dhVault.deposit{ value: 1 ether }();

        assertEq(dhVault.wstETHbalance(depositor1), 813_127_203_918_803_466);

        // Second depositor deposits 1 ETH
        vm.expectEmit();
        emit DiamondHandsVault.ETHDeposited(depositor2, 1 ether, 813_127_203_918_803_466);

        vm.prank(depositor2);
        dhVault.deposit{ value: 1 ether }();

        assertEq(dhVault.wstETHbalance(depositor2), 813_127_203_918_803_466);
    ...
    function test_withdraw_shouldSuccessfullyWithdrawAllTimeHighNotReached() public {
        ...
        vm.expectEmit();
        emit DiamondHandsVault.ETHWithdrawn(depositor1, 731_814_483_526_923_120, 81_312_720_391_880_346);

        vm.prank(depositor1);
        dhVault.withdraw();

        // User receives balance minus 10% fee
        vm.assertEq(IERC20(wstETH).balanceOf(depositor1), 731_814_483_526_923_120);
        // Fee recipient receives the 10% early exit fee
        vm.assertEq(IERC20(wstETH).balanceOf(exitFeeRecipient), 81_312_720_391_880_346);
    }
    ...
    function test_withdraw_shouldSuccessfullyWithdrawAllTimeHighReached() public {
        ...
        vm.expectEmit();
        emit DiamondHandsVault.ETHWithdrawn(depositor1, 813_127_203_918_803_466, 0);

        vm.prank(depositor1);
        dhVault.withdraw();

        // Full balance returned, no fee deducted
        vm.assertEq(IERC20(wstETH).balanceOf(depositor1), 813_127_203_918_803_466);
    ...
    function test_getETHPrice_shouldSuccessfullyGetETHPrice() public {
        ...
        vm.assertEq(price, 215_385_330_310);
        vm.assertEq(updatedAt, 1_774_065_719);
    }
